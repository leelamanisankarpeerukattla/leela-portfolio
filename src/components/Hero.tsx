...
import { Award, GitBranch, MapPin } from 'lucide-react';
...
export default function Hero() {
  ...
  return (
    <section id="home" className="container-shell relative overflow-hidden pb-12 pt-16 sm:pb-16 sm:pt-20">
      ...
      <div className="grid gap-8">
        <motion.div {...animation} transition={{ duration: 0.6, ease: 'easeOut' }}>
          ...
          <h1 className="max-w-5xl text-4xl font-black tracking-tight text-slate-950 dark:text-white sm:text-5xl lg:text-6xl">
            {profile.name}
          </h1>

          <p className="mt-5 max-w-3xl text-2xl font-semibold leading-tight text-slate-800 dark:text-slate-100 sm:text-[1.8rem]">
            {profile.headline}
          </p>
          ...
          <p className="mt-3 max-w-3xl text-sm font-medium leading-7 text-slate-500 dark:text-slate-400 sm:text-base">
            {profile.heroSecondary}
          </p>

          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="#projects" className="primary-button">
              <GitBranch size={17} aria-hidden="true" /> View Projects
            </Link>
            ...
            <Link href={profile.links.certifications} className="secondary-button" target="_blank" rel="noreferrer">
              <Award size={17} aria-hidden="true" /> Certifications
            </Link>
          </div>
        </motion.div>
      </div>
